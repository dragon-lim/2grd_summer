...
async def ask(client: AsyncAnthropic, question: str) -> str:
    response = await client.messages.create(
        model=MODEL,
        max_tokens=1000,
        # max_tokens: 답변 길이 상한. context window가 커도 토큰을 아끼려고 제한
...
#   → task 1개씩 처리, 겹치지 않아 6초 (의미 없는 비동기)
#   → 제어권을 event loop에 넘겨도 큐에 다른 작업이 없어 곧바로 돌아옴
# ═════════════════════════════════════════════
...
#   → 셋을 미리 큐에 넣어 대기가 겹침 → 약 3초
#   ※ 아래 'await result1, result2, result3' 는 사실 result1만 await되는 버그!
#     제대로 하려면 각각 await 하거나 gather로 묶어야 함
# ═════════════════════════════════════════════
# async def main():
#     start_time = time.time()
#     task1 = asyncio.create_task(make_order("라떼", 3))   # 시작만 (await 없이)
...
#   → LLM API 호출 = I/O 바운드(응답 대기) → 비동기로 겹치면 빨라짐
...
async def main():
    ...
    # gather = 세 질문을 동시에 보내고, 응답이 다 올 때까지 기다림
